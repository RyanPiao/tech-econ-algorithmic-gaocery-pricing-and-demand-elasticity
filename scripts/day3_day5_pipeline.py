#!/usr/bin/env python3
"""Generate Day 3-5 outputs from synthetic grocery pricing panel."""
...
def day3_outputs(df: pd.DataFrame) -> None:
    ...
    key_cols = ["effective_price", "conversion", "market_id", "week", "eta_minutes", "stockout_rate"]
    missingness = (
        df[key_cols]
        .isna()
        .mean()
    ...
    plt.title("Day 3: Conversion vs Effective Price Deciles")
...
def day4_outputs(df_clean: pd.DataFrame) -> pd.DataFrame:
    fs = cluster_fit(
        "ln_effective_price ~ did_treat_post + eta_minutes + stockout_rate + rain_index + C(market_id) + C(week) + C(hour_local)",
        df_clean,
    )
    rf = cluster_fit(
        "conversion ~ did_treat_post + eta_minutes + stockout_rate + rain_index + C(market_id) + C(week) + C(hour_local)",
        df_clean,
    )
    fe = cluster_fit(
        "conversion ~ ln_effective_price + eta_minutes + stockout_rate + rain_index + C(market_id) + C(week) + C(hour_local)",
        df_clean,
    )
...
def day5_outputs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    specs.append(
        (
            ...
            df.copy(),
            "conversion ~ ln_effective_price + major_campaign_window + other_price_experiment + eta_minutes + stockout_rate + rain_index + C(market_id) + C(week) + C(hour_local)",
        )
    )
    ...
    specs.append(
        (
            ...
            alt,
            "conversion ~ ln_total_paid + eta_minutes + stockout_rate + rain_index + C(market_id) + C(week) + C(hour_local)",
        )
    )
    ...
    specs.append(
        (
            ...
            win,
            "conversion ~ ln_effective_price_w + eta_minutes + stockout_rate + rain_index + C(market_id) + C(week) + C(hour_local)",
        )
    )
    ...
    specs.append(
        (
            ...
            placebo,
            "conversion ~ placebo_treat + eta_minutes + stockout_rate + rain_index + C(market_id) + C(week) + C(hour_local)",
        )
    )
...
def main() -> None:
    ...
    for p in sorted(OUT_DIR.glob("day*.csv")):
        print(" -", p)
    for p in sorted(OUT_DIR.glob("day*.png")):
        print(" -", p)
